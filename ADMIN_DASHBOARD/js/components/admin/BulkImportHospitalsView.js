/**
 * CareBridge Admin Dashboard — Bulk Import Hospitals Module
 * Located under Hospital Management.
 * Implements exact fields, validation rules, and schema from AddHospitalModal.
 */

import { adminStore } from '../../store/adminStore.js';

export const HOSPITAL_IMPORT_SCHEMA = {
  name: 'Hospitals',
  fileName: 'CareBridge_Hospitals_Import_Template.xlsx',
  idField: 'Hospital ID',
  headers: [
    'Hospital Name',
    'Hospital ID',
    ...
    'Emergency Desk / Ambulance Hotline',
  ...
  fieldRules: {
    'Hospital Type': {
      ...
      hint: 'District Hospital / Taluk Hospital / PHC / CHC'
    },
    'Sector / Ownership': {
      type: 'enum',
      allowed: ['Government', 'Private'],
      hint: 'Government / Private'
    },
    'District': {
      type: 'enum',
    ...
    'PIN Code': {
      type: 'pin',
      hint: '6-digit postal code (e.g. 636455)'
    },
    'Telephone Contact': {
      type: 'phone',
      hint: 'Official phone number (e.g. +91 427 2415000)'
    },
    'Official Health Email': {
      type: 'email',
      hint: 'Valid official email address'
    },
    'Total Sanctioned Beds': {
      type: 'number',
      min: 5,
      max: 5000,
      hint: 'Sanctioned bed count (5 - 5000)'
    },
    'ICU Beds': {
      type: 'number',
      min: 0,
      max: 500,
      hint: 'ICU beds count (>= 0)'
    },
    'Emergency / Triage Beds': {
      type: 'number',
      min: 0,
      max: 200,
      hint: 'Emergency beds count (>= 0)'
    },
    'General Ward Beds': {
      type: 'number',
      min: 0,
      max: 4500,
      hint: 'General ward beds (>= 0)'
    }
  },
  sampleData: [
    {
      'Hospital Name': 'Government Taluk Hospital, Omalur',
      'Hospital ID': 'HOSP-SLM-008',
      'Hospital Type': 'Taluk Hospital',
      'Sector / Ownership': 'Government',
      'Registration / License Number': 'TN-MED-SLM-4018',
      'District': 'Salem',
      'Taluk': 'Omalur',
      'City / Town / Village': 'Omalur Town',
      'PIN Code': '636455',
      'Complete Postal Address': 'Main Hospital Road, Near Bus Stand, Omalur, Salem - 636455',
      'Latitude': '11.7450',
      'Longitude': '78.0420',
      'Telephone Contact': '+91 4290 220100',
      'Official Health Email': 'th.omalur@tn.health.gov.in',
      'Emergency Desk / Ambulance Hotline': '108 / +91 4290 220108',
      'Total Sanctioned Beds': '150',
      'ICU Beds': '15',
      'Emergency / Triage Beds': '15',
      'General Ward Beds': '120',
      'Certified Clinical Services': 'OPD, Emergency, Pharmacy, Laboratory, X-Ray, Ultrasound, Ambulance, Telemedicine'
    },
    {
      'Hospital Name': 'Salem Community Health Centre, Attur',
      'Hospital ID': 'HOSP-SLM-009',
      'Hospital Type': 'Community Health Centre',
      'Sector / Ownership': 'Government',
      'Registration / License Number': 'TN-MED-SLM-5102',
      'District': 'Salem',
      'Taluk': 'Attur',
      'City / Town / Village': 'Attur Town',
      'PIN Code': '636102',
      'Complete Postal Address': 'Kamarajar Salai, Attur, Salem District - 636102',
      'Latitude': '11.5980',
      'Longitude': '78.5990',
      'Telephone Contact': '+91 4282 240500',
      'Official Health Email': 'chc.attur@tn.health.gov.in',
      'Emergency Desk / Ambulance Hotline': '108 / +91 4282 240999',
      'Total Sanctioned Beds': '60',
      'ICU Beds': '6',
      'Emergency / Triage Beds': '8',
      'General Ward Beds': '46',
      'Certified Clinical Services': 'OPD, Emergency, Pharmacy, Laboratory, Ambulance'
    }
  ]
};

export function createBulkImportHospitalsView() {
  const container = document.createElement('section');
  container.className = 'module-view-container bulk-import-view';
  container.setAttribute('aria-label', 'Bulk Import Hospitals Section');
  ...
  function downloadExcelTemplate() {
    ...
    try {
      ...
      const schema = HOSPITAL_IMPORT_SCHEMA;
      ...
      const instructionsData = [
        ['CareBridge Healthcare System — Hospital Master Bulk Import Guidelines'],
        [''],
        ...
        ['3. Columns match the exact fields required by the "+ Add Hospital" registration form.'],
        ['4. Hospital ID must be unique across the healthcare ecosystem.'],
        ['5. Certified Clinical Services should be comma-separated (e.g. OPD, Emergency, Pharmacy, Laboratory, Ambulance).'],
      ...
      schema.headers.forEach(header => {
        ...
        if (rule) {
          if (rule.type === 'enum') {
            ...
            ruleStr = 'Valid telephone with area or country code';
          } else if (rule.type === 'email') {
            typeStr = 'Email';
            ruleStr = 'Valid official email address';
          } else if (rule.type === 'number') {
            typeStr = 'Numeric Integer';
            ruleStr = `Min: ${rule.min}, Max: ${rule.max || 'N/A'}`;
      ...
      adminStore.showToast(`Downloaded hospital template: ${schema.fileName}`);
    } catch (err) {
      console.error('Hospital template download error:', err);
  ...
  function processExcelData(rawJson, headersFound) {
    ...
    const existingIds = new Set(adminStore.hospitals.map(h => h.hospitalId.toUpperCase()));
    ...
    rawJson.forEach((rowObj, index) => {
      ...
      // Hospital ID checks
      const hospId = sanitizedRow['Hospital ID'];
      if (hospId) {
        const idUpper = hospId.toUpperCase();
        if (seenIds.has(idUpper)) {
          rowErrors.push(`Duplicate Hospital ID (${hospId}) in file`);
        } else if (existingIds.has(idUpper)) {
          rowErrors.push(`Hospital ID (${hospId}) already registered in system`);
        } else {
          seenIds.add(idUpper);
        }
      }
      ...
      // Rules checks
      if (sanitizedRow['Official Health Email'] && !validateEmail(sanitizedRow['Official Health Email'])) {
        rowErrors.push('Official Health Email format is invalid');
      }

      if (sanitizedRow['Telephone Contact'] && !validatePhone(sanitizedRow['Telephone Contact'])) {
        rowErrors.push('Telephone Contact format is invalid');
      }
      ...
      const totalBeds = parseInt(sanitizedRow['Total Sanctioned Beds'], 10);
      if (sanitizedRow['Total Sanctioned Beds'] && (isNaN(totalBeds) || totalBeds < 5 || totalBeds > 5000)) {
        rowErrors.push('Total Sanctioned Beds must be between 5 and 5000');
      }
  ...
  function executeImport() {
    ...
    setTimeout(() => {
      try {
        ...
        validRows.forEach(r => {
          const rec = r.data;
          const servicesList = rec['Certified Clinical Services']
            ? rec['Certified Clinical Services'].split(',').map(s => s.trim()).filter(Boolean)
            : ['OPD', 'Emergency', 'Pharmacy'];

          const hospitalData = {
            name: rec['Hospital Name'],
            hospitalId: rec['Hospital ID'],
            type: rec['Hospital Type'] || 'Taluk Hospital',
            ownership: rec['Sector / Ownership'] || 'Government',
            registrationNumber: rec['Registration / License Number'],
            state: 'Tamil Nadu',
            district: rec['District'] || 'Salem',
            taluk: rec['Taluk'] || 'Salem',
            cityVillage: rec['City / Town / Village'],
            pinCode: rec['PIN Code'],
            address: rec['Complete Postal Address'],
            latitude: parseFloat(rec['Latitude']) || 11.6643,
            longitude: parseFloat(rec['Longitude']) || 78.1460,
            phone: rec['Telephone Contact'],
            email: rec['Official Health Email'],
            emergencyContact: rec['Emergency Desk / Ambulance Hotline'] || '108',
            totalBeds: parseInt(rec['Total Sanctioned Beds'], 10) || 100,
            icuBeds: parseInt(rec['ICU Beds'], 10) || 10,
            emergencyBeds: parseInt(rec['Emergency / Triage Beds'], 10) || 10,
            generalBeds: parseInt(rec['General Ward Beds'], 10) || 80,
            services: servicesList,
            status: 'Pending Verification',
            assignedAdminId: null,
            assignedAdminName: 'Pending Verification'
          };
        ...
        adminStore.logAudit(
          ...
          'Hospital Management',
          `${count} hospitals`,
          `Bulk imported ${count} hospitals via Excel template`
        );
  ...
  function render() {
    ...
    container.innerHTML = `
      ...
      <div class="module-header-container">
        <div class="module-title-group">
          <div style="display: flex; align-items: center; gap: 0.6rem;">
            <button type="button" class="btn-back-link" id="btn-back-to-hospitals" title="Return to Hospital Management">
            ...
            <h2 class="module-main-title">Bulk Import Hospitals</h2>
          </div>
          <span class="module-subtitle">Import multiple hospital records using an Excel template. Fields conform exactly to the "+ Add Hospital" accreditation schema.</span>
        </div>
      </div>
      ...
      <div class="bulk-import-grid">
        ...
        <div class="bulk-card">
          <div class="bulk-card-header">
            ...
            <div>
              <h3 class="bulk-card-title">Download Hospital Excel Template</h3>
              <p class="bulk-card-desc">Download the official template matching all fields from the "+ Add Hospital" registration form.</p>
            </div>
          </div>

          <div style="margin-top: 0.5rem;">
            <button type="button" class="btn-primary-action btn-download-template" id="btn-download-hosp-template">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/>
                <polyline points="7 10 12 15 17 10"/>
                <line x1="12" y1="15" x2="12" y2="3"/>
              </svg>
              <span>Download Hospital Excel Template (.xlsx)</span>
            </button>
          </div>
          ...
          <div class="bulk-instructions-box" style="margin-top: 1.5rem;">
            ...
            <ul class="bulk-checklist">
              ...
              <li><span class="chk-icon">✓</span> Hospital ID must be unique (e.g. HOSP-SLM-008).</li>
              <li><span class="chk-icon">✓</span> Total beds must be between 5 and 5000.</li>
              <li><span class="chk-icon">✓</span> Save the file as .xlsx before uploading.</li>
            </ul>

            <div class="bulk-fields-legend">
              <span class="legend-title">Columns matching "+ Add Hospital" form:</span>
        ...
        <div class="bulk-card">
          <div class="bulk-card-header">
            ...
            <div>
              ...
              <p class="bulk-card-desc">Drag and drop your populated hospital spreadsheet or browse from your computer.</p>
            </div>
          </div>

          ${!uploadedFile ? `
            <div class="bulk-dropzone" id="bulk-dropzone" tabindex="0" role="button" aria-label="Upload Hospital Excel File drag and drop area">
            ...
            ${validationSummary ? `
              ${validationSummary.errorCount > 0 || validationSummary.errors.length > 0 ? `
                <div class="bulk-validation-alert error" role="alert">
                  ...
                  <p class="val-alert-body">
                    ...
                    ${validationSummary.errorCount > 0 ? `<div>${validationSummary.errorCount} hospital row(s) contain invalid or missing required fields. These rows will be skipped if you proceed.</div>` : ''}
                  </p>
              ...
              ` : `
                <div class="bulk-validation-alert success" role="alert">
                  ...
                  <p class="val-alert-body">All ${validationSummary.total} hospital records conform to "+ Add Hospital" requirements and are ready for import.</p>
                </div>
              `}
            ` : `
              <div class="loading-state-box">
                <div class="cb-spinner"></div>
                <span>Parsing and validating hospital records...</span>
              </div>
            `}
          `}
        </div>
        ...
        <div class="bulk-preview-section">
          <div class="preview-header-bar">
            <div>
              ...
              <span class="preview-subtitle">Verify hospital records before committing to the CareBridge master database.</span>
            </div>
            ...
            <div class="preview-action-btn-group">
              ...
              >
                ...
                ` : `
                  ...
                  <span>Import Hospitals (${validationSummary.valid})</span>
                `}
              </button>
            </div>
          </div>
          ...
          <div class="data-table-wrapper preview-table-container">
            <table class="cb-table preview-table" id="hospitals-preview-table">
              <thead>
                <tr>
      ...
      ${importCompleted && importResults ? `
        <div class="bulk-result-card">
          ...
          <p class="result-subtitle">Hospital records have been verified and added to Hospital Management for accreditation.</p>
          ...
          <div class="result-action-buttons">
            ${importResults.skipped > 0 ? `
              <button type="button" class="btn-secondary-action" id="btn-view-hosp-error-report">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                <span>View Error Report (${importResults.skipped})</span>
              </button>
            ` : ''}

            <button type="button" class="btn-secondary-action" id="btn-goto-hosp-list">
            ...
            <button type="button" class="btn-primary-action" id="btn-import-another-hosp">
      ...
      <div id="hosp-error-modal-container"></div>
    `;

    // Attach handlers
    const btnBack = container.querySelector('#btn-back-to-hospitals');
    if (btnBack) {
      btnBack.addEventListener('click', () => {
        adminStore.setNavigation('hospitals', 'all');
      });
    }

    const btnDownload = container.querySelector('#btn-download-hosp-template');
    if (btnDownload) {
    ...
    const btnImport = container.querySelector('#btn-import-hospitals');
    if (btnImport) {
      btnImport.addEventListener('click', executeImport);
    }

    const btnImportAnother = container.querySelector('#btn-import-another-hosp');
    if (btnImportAnother) {
      btnImportAnother.addEventListener('click', resetImportState);
    }

    const btnGotoList = container.querySelector('#btn-goto-hosp-list');
    if (btnGotoList) {
    ...
    const btnViewErrors = container.querySelector('#btn-view-hosp-error-report');
    if (btnViewErrors) {
  ...
  function renderErrorReportModal() {
    ...
    const modalRoot = container.querySelector('#hosp-error-modal-container');
    if (!modalRoot) return;

    const modalBackdrop = document.createElement('div');
    modalBackdrop.className = 'admin-modal-backdrop';
    modalBackdrop.innerHTML = `
      <div class="admin-modal-shell" style="max-width: 650px;" role="dialog" aria-modal="true" aria-labelledby="modal-hosp-error-title">
        <div class="admin-modal-header">
          <div>
            <h3 class="admin-modal-title" id="modal-hosp-error-title">Hospital Bulk Import Error Report</h3>
            <span class="admin-modal-desc">${importResults.skipped} hospital records failed validation constraints.</span>
          </div>
          <button type="button" class="admin-modal-close" id="btn-close-hosp-error" aria-label="Close modal">
              ...
              <div class="error-report-item">
                ...
                <div class="err-item-content">
                  ...
                  <div class="err-item-summary">
                    <span>Hospital: <strong>${errRow.data['Hospital Name'] || 'Unnamed'}</strong> (${errRow.data['Hospital ID'] || 'No ID'})</span>
                  </div>
                </div>
              </div>
            `).join('')}
        ...
        <div class="admin-modal-footer">
          <button type="button" class="btn-primary-action" id="btn-dismiss-hosp-error">Done</button>
        </div>
      </div>
    `;
    ...
    modalBackdrop.querySelector('#btn-close-hosp-error')?.addEventListener('click', handleClose);
    modalBackdrop.querySelector('#btn-dismiss-hosp-error')?.addEventListener('click', handleClose);
  }
